...
def class_preparation(train_name, order: dict, new_names: dict):
    ...
    for file_path in files:
        ...
        name_suffix = filename.find('_1_participant_classification_report.csv')
        ...
        pred_file_path = os.path.join(
            folder_path,
            f'{test_type}_1_participant_predictions.csv'
        )
...
def heatmaps_preparation(participant: str):
    ...
    merged = tabulate(merged, headers='keys', tablefmt='psql')
...
def heatmaps_plot(grouping: str, data: pd.DataFrame):
    ...
    Args:
        grouping (str): 'sex', 'age', or 'diagnosis'
    ...
    for group_value, group in groups:
        ...
        for i, (_, participant_data) in enumerate(group.iterrows()):
            ...
            # Vectorized approach to create the agreement matrix
            agreement_matrix = (participant_matrix.T == participant_matrix.T).astype(int) * 50  # Agreement = 50 for now
            agreement_matrix[participant_matrix.T != true_diagnosis] = 0  # Disagreement = 0
            agreement_matrix[participant_matrix.T == true_diagnosis] = 100  # Correct agreement = 100
            ...
            axes[i].set_title(f"Participant {participant_data['ID']} (True: {true_diagnosis})")
            axes[i].set_xlabel("")
            axes[i].set_ylabel("")
