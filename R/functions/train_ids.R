train_ids <- function(meta){
  ...
  #How many participants should go into hold-out set from each subgroup (gender/diagnosis/IQ)
  ...
  #na.rm since there is one instance of NA IQ. Because it is only one, we figured, this was a ok method
  male_asd2 <- male_asd %>% 
  ...
  # this are the 80% of participants that are always used for training:
