...
source("functions/acoustics_preprop2.R")
...
```{r removing NAs}
# obs remove this column if everything is run again - it is now incorporated in acoustic_preprop2

Q_fam <- na.omit(Q_fam)
Q_unfam <- na.omit(Q_unfam)
MG <- na.omit(MG)
```
...
MG <- read_delim("/work/bachelor/Anna/partitioned/train_MatchingGame.csv", delim = ";", show_col_types = FALSE)
Qfam <- read_delim("/work/bachelor/Anna/partitioned/train_Questions_fam.csv", delim = ";", show_col_types = FALSE)
Qunfam <- read_delim("/work/bachelor/Anna/partitioned/train_Questions_unfam.csv", delim = ";", show_col_types = FALSE)

# making lists of test sets belonging to each model
all <- list.files("/work/bachelor/Anna/partitioned", full.names = TRUE)
...
```{r, elastic net}
elastic(MG_model, 3, "MatchingGame")
elastic(Qfam_model, 3, "Questions_fam")
elastic(Qunfam_model, 3, "Questions_unfam")

```

```{r}
c <- read_delim("/work/bachelor/data/Preprop_data_w_folds/MatchingGame_w_3folds.csv", delim = ";")
```
